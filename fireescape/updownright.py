    ...
    while running:
        ...
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                # ===== 수동 이동 로직 (auto_planning 모드일 때만 작동) =====
                if auto_planning:
                    r, c = agent_pos
                    new_pos = None
                    if e.key == pygame.K_UP: new_pos = (r - 1, c)
                    elif e.key == pygame.K_DOWN: new_pos = (r + 1, c)
                    elif e.key == pygame.K_LEFT: new_pos = (r, c - 1)
                    elif e.key == pygame.K_RIGHT: new_pos = (r, c + 1)
                    
                    if new_pos is not None:
                        new_r, new_c = new_pos
                        if inb(new_r, new_c, current_rows, current_cols) and not blocked[new_r][new_c]:
                            agent_pos = new_pos
                            ...
                            if agent_pos in goals and not just_teleported:
                                ...
                                if 0 <= partner_idx < len(goals):
                                    agent_pos = goals[partner_idx]
                                    just_teleported = True
                            elif agent_pos in escapes:
                                print("탈출 성공!"); auto_planning = False
