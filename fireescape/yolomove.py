...
def main():
    ...
    while running:
                ...
                if current_bald_pos and prev_bald_pos:
                    ...
                    if abs(dr) > abs(dc):  # Dominant vertical movement
                        if dr > 0:
                            ...
                            next_agent_pos = (agent_pos[0] - 1, agent_pos[1])
                    else:  # Dominant horizontal movement
                        if dc > 0:
        ...
        if path and agent_pos in path:
            try:
                current_index = path.index(agent_pos)
                if current_index + 1 < len(path):
                    ...
                    if dr == 1 and dc == 0:
                        direction_to_send = "DOWN"
                    elif dr == -1 and dc == 0:
                        direction_to_send = "UP"
                    elif dr == 0 and dc == 1:
                        direction_to_send = "RIGHT"
                    elif dr == 0 and dc == -1:
                        direction_to_send = "LEFT"
            except ValueError:
                pass # agent_pos not in path
        
        ws_handler.set_direction(direction_to_send)
