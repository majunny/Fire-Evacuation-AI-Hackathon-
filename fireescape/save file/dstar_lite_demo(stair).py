    ...
    while running:
        ...
        if fire_cells and auto_planning:
            ...
            if fire_step_timer >= FIRE_STEP_INTERVAL:
                ...
                # ----- 수정된 부분 시작 -----
                # 새로 번진 불이 목표(계단)에 도달했는지 확인하고, 반대편으로도 번지게 함
                teleported_fire = set()
                    ...
                    for f_pos in new_fire_cells:
                        if f_pos in goals:
                            try:
                                ...
                                if reached_goal_idx < num_goal_colors:
                                    partner_idx = reached_goal_idx + num_goal_colors
                                else:
                                    partner_idx = reached_goal_idx - num_goal_colors

                                if 0 <= partner_idx < len(goals):
                                    partner_pos = goals[partner_idx]
                                    # 반대편이 이미 불타고 있거나 벽이 아니면 불을 추가
                                    if partner_pos not in fire_cells and not blocked[partner_pos[0]][partner_pos[1]]:
                                        teleported_fire.add(partner_pos)
                                        print(f"불이 계단({reached_goal_idx+1})을 통해 반대편({partner_idx+1})으로 번졌습니다!")
                            except ValueError:
                                continue # goals에 없는 경우 무시
                
                # 순간이동한 불을 새로운 불 목록에 추가
                if teleported_fire:
                    new_fire_cells.extend(list(teleported_fire))
                # ----- 수정된 부분 끝 -----
        ...
        if planning and path and auto_planning:
            # 에이전트가 현재 경로의 끝에 도달했는지 확인
            # ----- 수정된 부분 시작 -----
            # 방금 텔레포트하지 않았을 경우에만 텔레포트 로직 실행
            if agent_pos == path[-1] and agent_pos in goals and not just_teleported:
            # ----- 수정된 부분 끝 -----
                reached_goal_idx = goals.index(agent_pos)
                
                # 짝이 되는 목표(계단)의 인덱스 계산
                # 5개의 색상과 10개의 목표는 (0,5), (1,6)... 쌍을 이룸
                num_goal_colors = len(GOAL_COLORS)
                if reached_goal_idx < num_goal_colors:
                    teleport_partner_idx = reached_goal_idx + num_goal_colors
                else:
                    teleport_partner_idx = reached_goal_idx - num_goal_colors

                # 짝이 되는 목표가 유효한지 확인
                if 0 <= teleport_partner_idx < len(goals):
                    teleport_destination = goals[teleport_partner_idx]
                    print(f"계단({reached_goal_idx + 1}) 도착! 반대편 계단({teleport_partner_idx + 1})으로 이동합니다.")
                    agent_pos = teleport_destination  # 에이전트 위치를 순간이동시킴
                    
                    # ----- 수정된 부분 시작 -----
                    just_teleported = True # 텔레포트 플래그 설정
                    # ----- 수정된 부분 끝 -----

                    # 이동 후, 새 위치에서 다시 최적의 목표를 탐색
                    planner = None  # 새 계획을 위해 기존 planner 초기화
                    active_goal_idx, planner, plan_gen = auto_replan(
                        blocked, agent_pos, goals, fire_cells, planner, current_rows, current_cols
                    )
                    ...
                    if planner is not None:
                        ...
                        planning = True
                        step_timer = 0.0
                        print(f"이동 후, 새로운 최적 목표({active_goal_idx + 1})를 향해 경로 탐색을 시작합니다.")
                    else:
                        planning = False
                        path = []
                        print("이동 후, 도달 가능한 목표를 찾을 수 없습니다.")
                else:
                    # 짝이 없는 경우 (예: 목표 개수가 홀수)
                    print("목표 도달. 짝이 되는 반대편 계단이 없어 정지합니다.")
                    planning = False
                    path = []
            
            # ----- 수정된 부분 시작 -----
            # 경로의 끝에 도달했지만 목표가 아니거나, 이미 텔레포트한 경우
            elif agent_pos == path[-1]:
                 print("경로의 끝에 도달했습니다. 정지합니다.")
                 planning = False
                 path = []
            # ----- 수정된 부분 끝 -----
            else:
                ...
                if step_timer >= STEP_INTERVAL:
                    ...
                    if not replan_needed and current_path_idx + 1 >= len(path):
                        print("경로 끝 도달 (로직 오류) → 재계획"); replan_needed = True
                    ...
                    if replan_needed:
                        ...
                        if planner:
                             ...
                             if new_path:
                                ...
                                print("기존 목표로의 우회 경로 없음 → 다른 목표 탐색")
                                active_goal_idx, planner, plan_gen = auto_replan(
                                    blocked, agent_pos, goals, fire_cells, None, current_rows, current_cols
                                )
                                if planner:
                                    path = planner.get_path()
                                    print(f"새로운 목표 {active_goal_idx + 1}(으)로 전환합니다.")
                                else:
                                    planning = False
                                    path = []
                                    print("우회 및 새로운 목표 탐색에 실패했습니다.")
                        step_timer = 0.0
                    else:
                        # 다음 위치로 에이전트 이동
                        ...
                        # ----- 수정된 부분 시작 -----
                        # 에이전트가 목표 지점이 아닌 곳으로 이동하면, 다시 텔레포트할 수 있도록 플래그를 리셋
